...
public class ChangeEvTask extends AsyncTask<Void, Void, Integer> {
    ...
    public ChangeEvTask(Context context, int inputEv, int inSpeechVolume) {
    ...
    protected void onPostExecute(Integer result) {
        ...
        if ( (EV_MIN <= result) && (result <= EV_MAX) ) {
            new SoundManagerTask(context, soundList[result], speechVolume).execute();
        } else if (result==-2) {
            new SoundManagerTask(context, R.raw.ev_error, speechVolume).execute();
        } else {
            //実行不可は無処理
        }
    }
